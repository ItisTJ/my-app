...
const PlaceOrder = () => {
  ...
  useEffect(() => {
    ...
    const region = regions.find((r) => r.name.toLowerCase() === city.toLowerCase())
    if (region) {
      setCalculatedShipping(region.rate)
    } else {
      setCalculatedShipping(regions[0].rate) // Default to Colombo if not found
    }
  ...
  const processOrder = () => {
    ...
    // Calculate discount only if itemsPrice is greater than $500
    const discount = itemsPrice > 500 ? itemsPrice * 0.1 : 0

    // Calculate total price: itemsPrice - discount + shipping
    // Convert to number to ensure it meets validation requirements
    ...
    createOrder({
      ...
      discount: Number(discount.toFixed(2)), // Add discount to the order data, as a number
    })
        ...
        <div className="md:w-1/3">
          <div className="bg-white shadow-xl rounded-2xl p-6">
              ...
              <div className="flex justify-between mt-2 font-bold">
                ...
                <span>
                  ...
                  {(
                    ...
                    (cart.data.itemsPrice > 500 ? cart.data.itemsPrice * 0.1 : 0) +
                    calculatedShipping
                  ).toFixed(2)}
                </span>
              </div>
            ...
            <Link href="/" passHref>
              <button
                type="button"
                className="text-gray-600 hover:text-teal-600 text-sm font-medium transition-colors inline-flex items-center mt-4"
              >
                <ArrowLeft size={16} />
                Back to Shopping
              </button>
