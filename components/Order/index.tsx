...
const Order: React.FC<OrderProps> = ({ pageId }) => {
  ...
  const onPaymentHandler = ({
    ...
    update_time,
    status,
  }: { id: string; payer: { email_address: string }; update_time: string; status: string }) => {
    const paymentResult = {
      id,
      email_address,
      update_time,
          ...
          <ListGroup variant="flush">
            ...
            <ListGroup.Item>
              ...
              {data.orderItems.length === 0 ? (
                ...
                <ListGroup variant="flush">
                  {data.orderItems.map((item: { image: string; name: string; productId: string; qty: number; price: number }, index: number) => (
