  ...
  return async (dispatch) => {
    try {
      ...
      for (const policy of policyData) {
        await axios.post("http://localhost:4000/api/privacy-policy", policy, config);
      }
...
export const fetchPrivacyPolicies = (): ThunkAction<Promise<void>, RootState, unknown, AnyAction> => {
  return async (dispatch) => {
    try {
      ...
      const { data } = await axios.get("http://localhost:4000/api/privacy-policy");
...
export const deletePrivacyPolicy = (
  ...
  return async (dispatch) => {
    try {
      ...
      await axios.delete(`http://localhost:4000/api/privacy-policy/${id}`);
  ...
  return async (dispatch) => {
    try {
      ...
      const { data } = await axios.put(
        `http://localhost:4000/api/privacy-policy/${id}`,
        updatedPolicyData,
        config
      );
